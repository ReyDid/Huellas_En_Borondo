...
public class Cntrl_Camara : MonoBehaviour
{
    ...
    void Rotacion()
    {
        Quaternion Inclncn = _Rotacion.rotation;
        Inclncn.eulerAngles = new Vector3(_Encuadre.Angl_X + -(dstncrObjtvCntrY * 2f), _Encuadre.Angl_Y + -(dstncrObjtvCntrX * 1.1f), _Encuadre.Angl_Z);

        _Rotacion.rotation = Quaternion.Lerp(_Rotacion.rotation, Inclncn, VlcdRtcn * Time.deltaTime);
    }
    void Mover()
    {
